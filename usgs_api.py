...
def api_code(site_no,begin_date,end_date,data_type):
    ...
    elif(data_type == "Daily") or (data_type == 'daily'):
...
def get_data(site_no,begin_date,end_date):
    ...
    if(data_type == '15min'):
        ...
        '''
        data1="Agency,Site_ID,Date/Time,TZ,Flow (cfs),Indic\n"+"\n".join([','.join([x for x in y.split('\t')]) for y in tempu])
    
        '''
        Use when we need daily data
        '''
    else:
        data1="Agency,Site_ID,Date/Time,Flow (cfs),Indic\n"+"\n".join([','.join([x for x in y.split('\t')]) for y in tempu])
    ...
    if(flow_info['Flow (cfs)'].isnull().sum()>=1): #Handles the portion where date is present but flow value is missing
      
        print('Flow column data missing in NWIS database')
        flow_info = flow_info.mask(flow_info=='NaN').ffill()
        print('Missing data filled')    
    '''
